...
//        self.GetLove()
        ...
        query?.getFirstObjectInBackground(block: { user, error in
            if user != nil{
                withAnimation {
                    ...
                    let client = Client(username: username, clientname: clientname, introduction: introduce, sex:sex, locate: locate, clientimage: clientimage.url!, clientbackground: backgrounimage.url!,birthday: brithday,school: school)
                    withAnimation {
    ...
    func GetMyPinLun(){
        ...
        let pre = NSPredicate(format: "pinlunname = '\(self.Myclientdata.MyClient.username)'")
        let query = PFQuery(className: "PinLun",predicate: pre)
    ...
    func GetLove(){
        ...
        query.findObjectsInBackground { obj, error in
            if obj != nil{
                print("Love\(String(describing: obj))")
                withAnimation {
                    self.LoveMyself = []
                }
                for i in obj!{
                    ...
                    if objectid == self.Myclientdata.MyClient.username{
                        ...
                        let time = i.createdAt!
                        let query = PFUser.query()
                        query?.whereKey("username", equalTo: myobjectid)
                        query?.getFirstObjectInBackground(block: { user, error in
                            if user != nil{
                                let clientname = user!.object(forKey: "username") as! String
                                let introduce = user!.object(forKey: "introduction") as! String
                                ...
                                let school = user!.object(forKey: "school") as! String
                                let brithday = user!.object(forKey: "birthday") as! Date
                                let client =  Client(username: myobjectid, clientname: clientname, introduction: introduce, sex:sex, locate: locate, clientimage: clientimage.url!, clientbackground: backgrounimage.url!,birthday: brithday,school: school)
                                withAnimation {
